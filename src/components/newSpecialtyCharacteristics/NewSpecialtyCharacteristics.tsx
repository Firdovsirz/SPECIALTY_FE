...
import { addSpecialtychar, SpecialtyCharPayload } from "../../services/specialtCharacteristics/specialtyChar";

export default function NewSpecialtyCharacteristics() {
    ...
    const createSpecialtyChar = async () => {
        try {
            setLoading(true);
            const charPayload: SpecialtyCharPayload = {
                specialty_code: specialtyCode,
                program_desc: ploContent,
                degree_requirements: requirements
            }
            const result = await addSpecialtychar(charPayload, token ? token : "");
    ...
    return (
        <div>
            ...
            <TextArea
                value={ploContent}
                placeholder="Proqram təlim məqsədləri"
                onChange={(value) => setRequirements(value.charAt(0).toUpperCase() + value.slice(1))}
                className="mb-[10px]"
            />
